...
from pyrogram.types import ChatMemberUpdated, Message, InlineKeyboardButton, InlineKeyboardMarkup
from pyrogram.errors import InviteRequestSent, ChatWriteForbidden
...
async def awelcome_command(client, message: Message):
    ...
    # Check assistant admin privileges
...
async def greet_new_members(client: Client, member: ChatMemberUpdated):
    ...
    try:
        # Hanya greet member baru
        if member.new_chat_member and (not member.old_chat_member or member.old_chat_member.status == "left"):
